...
A property of the `SymbolColor` enum that returns a unique identifier for the color case.

## Overview

The `id` property returns a `Double` value that uniquely identifies a `SymbolColor` case, enabling its use in SwiftUI lists or data models that require `Identifiable` conformance. Each predefined color has a fixed `id` from 0 to 14. The table below displays the `value`, `name`, `color`, and `id` properties for each predefined color, with the `id` property corresponding to the ID column.
