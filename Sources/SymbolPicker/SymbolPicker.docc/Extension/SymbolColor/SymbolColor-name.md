...
A property of the `SymbolColor` enum that returns the human-readable name of the color.

## Overview

The `name` property returns a `String` representing the display name of a `SymbolColor` case, such as "Red" or "Blue". This is useful for presenting color names in UI elements like pickers or labels. The table below displays the `value`, `name`, `color`, and `id` properties for each predefined color, with the `name` property corresponding to the color name column.
