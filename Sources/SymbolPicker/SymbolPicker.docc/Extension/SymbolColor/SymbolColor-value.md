...
A property of the `SymbolColor` enum that returns the RGBA color components as an array of doubles.

## Overview

The `value` property provides the RGBA components of a `SymbolColor` case as an array of four `Double` values, representing red, green, blue, and alpha (each in the range `[0, 1]`). For predefined colors, it returns fixed values defined in the enum. The table below displays the `value`, `name`, `color`, and `id` properties for each predefined color, with the `value` property corresponding to the RGBA values column.
